...
# Modified by: Jay OConnor <jaydoconnor@gmail.com> https://gist.github.com/jdoconnor/32ef0314cecb80157c4e
# Modified again by: Bryan Kendall <bryan@runnable.com>
####
...
if [ "$REPO_TYPE" == "true" ]
then
  echo -e "${STYLE_BOLD}${COLOR_ERROR}Cannot run this script on a bare Repository${STYLE_RESET}" && exit 1
fi
...
updateFileTimeStamp()
{
  ...
  then
    ...
    echo "${STYLE_BOLD}${COLOR_ERROR}Unknown Operating System to perform timestamp update${STYLE_RESET}" >&2
    exit 1
  fi
}
